...
function AddDivFilms() {
    ...
    for (let i = 0; i < movielist.length; i++) {
        ...
        filmscardsList += "<div href=\"#top\" class='card_body' onclick=seedetails('" + i + "')>"
            + `<div class="films_name" style="text-align: center"><h3><b>${info.title} </b></h3></div>`
            // /div card-body
            + '</div>'
    }
...
function seedetails(placefilm) {
    ...
    let detaillist2 = "";
    let detailscroll = "";
    let info = movielist[placefilm];
    console.log(info);
    // let planetfilm = (info.homeworld.slice(-2, -1) - 1); //gets last character
    // let infop = planetlist[planetfilm];
    // console.log(infop);
    planetfilm = info.planets;
    console.log(planetfilm);
    peoplefilm = info.characters;
    console.log(peoplefilm);

    detaillist = '<div class="people_info">'
        + '<div class="people_name"><h3><b>' + info.title + '</b></h3>' + '</div>'
        + '<div class="people_img"><img src="picture/film/' + placefilm + '.jpg" style= "width: 75%; border: solid 2px yellow">' + '</div>'
        + '<div class="people_gender"><p><span class="bold">Director : </span>' + info.director + '</p>' + '</div>'
        + '<div class="people_height"><p><span class="bold">Date : </span>' + info.release_date + '</p>' + '</div>'
        + '</div>'
        + '<div class="people_planet">'
        + '<p>Planêtes : </p></div>'

    for (let m = 0; m < planetfilm.length; m++) {
        let nindice = (m).toString(); // convertit m en caractère
        let zindice = planetfilm[nindice]; // selectionne le film à nindice
        let moviecar = (zindice.slice(-2, -1) - 1); // récupère le numéro du film
        let infop = planetlist[moviecar]; // va chercher les infos du film par rapport à son numéro
        if (infop != undefined) {
        detaillist += '<button onclick="window.location.href=\'Planet.html\';">' + infop.name + '</button>'
        } else {}
    }
    detaillist2 = '<div class="people_perso">'
        + '<p>Personnages : </p></div>'

    for (let m = 0; m < 5; m++) {
        let nindice = (m).toString(); // convertit m en caractère
        let zindice = peoplefilm[nindice]; // selectionne le film à nindice
        let moviecar = (zindice.slice(-2, -1) - 1); // récupère le numéro du film
        let infoc = peoplelist[moviecar]; // va chercher les infos du film par rapport à son numéro
        console.log(infoc);
        if (infoc != undefined) {
        detaillist2 += '<button onclick="window.location.href=\'People.html\';">' + infoc.name + '</button>'
        } else{}
    }
